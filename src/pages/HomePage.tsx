...
export const HomePage: React.FC = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-12 sm:space-y-16 animate-fadeIn">
      {/* Top Greeting Header with current date & glitter ✦ */}
      <section className="space-y-1.5">
        <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-1">
          <h1 className="font-serif italic font-semibold text-4xl sm:text-5xl text-light-text dark:text-night-text tracking-tight flex items-center gap-2.5">
            <span>{getGreeting()}</span>
            <span className="text-pastel-yellow-ink dark:text-pastel-yellow not-italic inline-block text-3xl sm:text-4xl animate-soft-pulse">✦</span>
          </h1>
          <span className="font-serif italic text-sm sm:text-base text-light-muted dark:text-night-muted">
      ...
      {/* SECTION 3: THINGS I GOT TO DO (Visually Quieter) */}
      <section className="p-5 sm:p-6 rounded-3xl bg-light-surface/60 dark:bg-night-surface/60 border-2 border-light-border/70 dark:border-night-border/70 flex items-center justify-between">
        <div>
          ...
          <p className="font-serif italic font-semibold text-xl sm:text-2xl text-light-text dark:text-night-text">
            {activeTasksCount} {activeTasksCount === 1 ? 'thing' : 'things'} saved
          </p>
        </div>

        <button
          onClick={() => setPage('tasks')}
          className="inline-flex items-center gap-2 px-4 py-2 rounded-2xl bg-light-bg dark:bg-night-elevated text-xs sm:text-sm font-semibold text-light-text dark:text-night-text border-2 border-light-border dark:border-night-border hover:border-pastel-sage transition-all btn-clean"
        >
          <span>view all</span>
        ...
        {forFunItems.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            {forFunItems.slice(0, 2).map((item) => (
              <div
                key={item.id}
                className="group p-5 rounded-2xl bg-light-surface/75 dark:bg-night-surface/75 border-2 border-light-border/70 dark:border-night-border/70 hover:border-pastel-pink/70 transition-all duration-200 flex items-center justify-between"
              >
                <div>
                  <h4 className="font-serif italic font-semibold text-xl sm:text-2xl text-light-text dark:text-night-text">
                    {item.title}
                  </h4>
                  {item.duration && (
                    <span className="text-xs text-light-muted dark:text-night-muted font-normal">
                ...
                <button
                  ...
                  className="p-2 rounded-xl text-light-muted dark:text-night-muted hover:text-pastel-pink-ink dark:hover:text-pastel-pink hover:bg-light-bg dark:hover:bg-night-elevated border border-transparent hover:border-light-border dark:border-night-border transition-colors btn-clean"
                  title="Enjoy now"
                >
                  <Play className="w-4 h-4 fill-current" />
                </button>
      ...
      <section className="p-5 sm:p-6 rounded-3xl bg-light-surface/40 dark:bg-night-surface/40 border-2 border-dashed border-light-border/80 dark:border-night-border/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-2xl bg-light-bg dark:bg-night-elevated text-xs sm:text-sm font-semibold text-light-text dark:text-night-text border-2 border-light-border dark:border-night-border hover:border-pastel-yellow transition-colors btn-clean"
        >
          <span>begin 2 min reset</span>
          <ArrowRight className="w-3.5 h-3.5 stroke-[2]" />
        </button>
      </section>
    </div>
